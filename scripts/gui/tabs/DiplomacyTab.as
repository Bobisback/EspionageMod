...
class DiplomacyTab : Tab {
	...
	void updateCardPosition(bool snap = false) {
		...
		//Card stack
		print("Stack panel size " + stackPanel.size.height);
...
class EmpireBox : BaseGuiElement {
	...
	EmpireBox(DiplomacyTab@ tab) {
		...
		@flag = GuiSprite(this, Alignment().padded(12,12,62,12));
		flag.horizAlign = 1.0;
	...
	void set(Empire@ empire) {
		...
		string tt = format("[color=$2][font=Medium]$1[/font][/color]", empire.name, toString(empire.color));
		for(uint i = 0; i < traitCnt; ++i) {
			if(tt.length != 0)
				tt += "\n\n";
