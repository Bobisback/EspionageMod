...
class GuiPlanetSurface : BaseGuiElement {
	...
	void drawSurfaceData() {
		...
		auto@ rng = RandomEngine((size.width << 16) | size.height);
		
		//array<vec2f> surfData(size.width * size.height);
		for(uint y = 0; y < size.height; ++y) {
			for(uint x = 0; x < size.width; ++x) {
				const Biome@ biome = surface.getBiome(x, y);
				//surfData[y*size.width + x] = vec2f(biome.humidity, biome.temp);
				
				if(biome.isVoid) {
					img.set(x,y, Color(rng.randomi(0,255), rng.randomi(0,255), 0, 0));
				}
				else if(!biome.isCrystallic) {
					float h = clamp(biome.humidity * rng.randomf(0.9f, 1.1f), 0.f, 1.f);
					float t = clamp(biome.temp * rng.randomf(0.9f, 1.1f), 0.f, 1.f);
					img.set(x,y, Color(h * 255.99f, t * 255.99f, 0, 255));
				}
				else {
					img.set(x,y, Color(rng.randomi(0,255), rng.randomi(0,255), 255, 255));
				}
			}
		}
		...
		@surfTex.material.texture3 = material::GuiPlanetSurface.texture2;
					...
					if(bld.type.size.x == 1 && bld.type.size.y == 1) {
						...
						for(uint rx = 0; rx < bld.type.size.x; ++rx) {
							...
							for(uint ry = 0; ry < bld.type.size.y; ++ry) {
								...
								if(tileFlags & SuF_Usable == 0 && bld.type.getMaintenanceFor(surface.getBiome(px, py)) != 0) {
...
void drawHoverBuilding(const Skin@ skin, const BuildingType@ type, const vec2i& mousePos, const vec2i&in gridPos, GuiPlanetSurface@ surface = null) {
	...
	//Draw each tile
	int buildCost = type.baseBuildCost;
			...
			if(surface !is null && gridPos.x != -1 && gridPos.y != -1) {
					...
					|| uint(rpos.y) >= surface.surface.size.y)
				{
					...
					if(!surface.surface.getBiome(rpos.x, rpos.y).buildable) {
						...
						double bld = type.tileBuildCost * biome.buildCost;
	...
	string cost = formatMoney(buildCost)+" / "+formatMoney(maintainCost);
