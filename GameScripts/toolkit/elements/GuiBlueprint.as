...
class GuiBlueprint : BaseGuiElement {
	...
	vec2i getGridPosition(const vec2i& offset, bool selectInactive = false) {
		...
		vec2d pos;
		pos.x = double(offset.x) / double(hexSize.x);
	...
	void drawElevation(const vec2u& hex, vec2i& pos, double hexFactor, vec2i& hexSize, bool floating = false, bool second=true) {
		int seg = floor(BORDER_HEIGHT * hexFactor);
	...
	void drawHexFloor(const vec2u& hex, const vec2i& atPos, const vec2i& size, bool floating = false) {
		...
		if(bp !is null) {
			uint index = uint(design.hexStatusIndex(hex));
			if(index != uint(-1)) {
				const HexStatus@ status = bp.getHexStatus(index);
				...
				if(status !is null && subsys.has(HV_HP)) {
					repairing = int(hex.x) == bp.repairingHex.x
						&& int(hex.y) == bp.repairingHex.y;

					if(prevDamage[index] > status.hp) {
						damagedAt[index] = frameTime;
						damaging = true;
						prevDamage[index] = status.hp;
					}
					else {
						damaging = damagedAt[index] > frameTime - 3.0;
					}
	...
	void drawHex(const vec2u& hex, const vec2i& atPos, const vec2i& size, bool floating = false, bool drawFloor = true) {
		bool active = hull.active[hex];
		if(!displayInactive && !active)
			return;
		...
		bool drawWalls = sys is null || (!sys.type.hasTag(ST_IsArmor) && !sys.type.hasTag(ST_NoWall));
		...
		if(bp !is null) {
			...
			if(index != uint(-1)) {
				...
				if(status !is null && sys.has(HV_HP)) {
					repairing = int(hex.x) == bp.repairingHex.x
						&& int(hex.y) == bp.repairingHex.y;
	...
	void drawAll() {
		...
		for(uint y = 0; y < height; ++y) {
			for(uint x = 0; x < width; x += 2)
				drawHex(vec2u(x, y), getHexPos(vec2u(x, y)) + start, hexSize, drawFloor=false);
			for(uint x = 1; x < width; x += 2)
				drawHex(vec2u(x, y), getHexPos(vec2u(x, y)) + start, hexSize, drawFloor=false);
		}
