...
Ship@ createShip(vec3d position, const Design@ design, Empire@ owner, Object@ groupLeader = null, bool free = false, bool memorable = false) {
	...
	if(design.hasTag(ST_Station))
		shipDesc.radius = stationRadiusFactor * pow(design.size,1.0/shipVolumePower);
	else
		shipDesc.radius = pow(design.size,1.0/shipVolumePower);
	shipDesc.position = position;
	if(memorable)
		shipDesc.flags |= objMemorable;
	
	MeshDesc shipMesh;
	@shipMesh.model = design.hull.model;
	@shipMesh.material = design.hull.material;
	@shipMesh.iconSheet = design.distantIcon.sheet;
	shipMesh.iconIndex = design.distantIcon.index;
...
Ship@ createShip(Object& at, const Design@ design, Empire@ owner = null, Object@ groupLeader = null, bool detectLeader = true, bool free = false, bool move = true, bool forceLeader = false) {
	...
	if(design.hasTag(ST_Support)) {
		...
		if(move) {
			if(planet !is null) {
				vec2d pos = random2d(planet.OrbitSize);
