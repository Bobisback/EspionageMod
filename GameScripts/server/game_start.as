...
void init() {
	...
	for(uint i = 0, cnt = galaxies.length; i < cnt; ++i) {
		galaxies[i].estPlayerCount = ceil(double(majorEmpireCount) / double(cnt));
	...
	{	
		...
		for(uint i = 0; i < homeworlds.length; ++i) {
			...
			for(uint j = 0; j < mapCnt && !search.map.canHaveHomeworld(emp); ++j)
				@search.map = galaxies[(mapN + mapC + j) % mapCnt];
			...
			double rad = search.map.radius * 0.4;
...
void generateNewSystem(const vec3d& pos, double radius, SystemGenerateHook@ hook, const string& name = "", bool makeLinks = true) {
