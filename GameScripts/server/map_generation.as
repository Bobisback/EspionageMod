...
class MapGeneration {
	...
	void calculateHomeworldDistances() {
		...
		for(uint i = 0, cnt = homeworlds.length; i < cnt; ++i)
			sDist(i, homeworlds[i], 0.0);
	}

	void sDist(uint index, SystemData@ dat, double dist) {
		dat.hwDistances[index] = dist;
		uint m = 0;
		for(uint i = 0, cnt = dat.adjacentData.length; i < cnt; ++i) {
			auto@ adj = dat.adjacentData[i];
			if(adj.hwDistances[index] > dist) {
				double d = dist + 1000.0 + dat.position.distanceTo(adj.position);
				if(adj.hwDistances[index] > d) {
					adj.hwDistances[index] = d;
					m |= 1<<i;
				}
			}
		}
		if(m != 0) {
			for(uint i = 0, cnt = dat.adjacentData.length; i < cnt; ++i) {
				if(m & (1<<i) != 0) {
					auto@ adj = dat.adjacentData[i];
					sDist(index, adj, adj.hwDistances[index]);
				}
			}
		}
	}
