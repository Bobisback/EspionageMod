...
final class ColonizationEvent : Savable, Serializable {
	Object@ from;
	Object@ to;
...
const Design@ getDefenseDesign(Empire& owner, double defenseRate, double tolerance = 1.0) {
...
final class AutoImport : Savable, AutoImportDesc {
	void save(SaveFile& file) {
		file << to;
		file << handled;
		if(cls !is null) {
...
final class ObjectManager : Component_ObjectManager, Savable {
		...
		while(defense > 0) {
			...
			if(defenseLabor <= 0) {
				...
				if(spawnAt !is null) {
					...
					Planet@ fallback = owner.Homeworld;
					if(fallback !is null && (!fallback.valid || fallback.owner !is owner))
						@fallback = null;
					if(spawnAt.isRegion)
	...
	void autoImportToLevel(Empire& emp, Object& into, uint level) {
		...
		array<int> remaining;
		resources.syncFrom(into.getResourcesFor(emp));
