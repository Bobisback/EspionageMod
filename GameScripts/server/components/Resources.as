...
final class ObjectResources : Component_Resources, Savable {
	...
	void save(SaveFile& file) {
		...
		cnt = queuedExports is null ? 0 : queuedExports.length;
		file << cnt;
		for(uint i = 0; i < cnt; ++i)
			file << queuedExports[i];

		cnt = queuedImports is null ? 0 : queuedImports.length;
		file << cnt;
		for(uint i = 0; i < cnt; ++i)
	...
	void getNativeResources(const Object& obj) {
		for(uint i = 0, cnt = nativeResources.length; i < cnt; ++i)
	...
	uint getUniqueFoodCount(int modBy = 0) {
		uint uniques = 0;
		const ResourceClass@ foodCls = getResourceClass("Food");
	...
	uint getResourceTargetLevel(Object& obj) {
		//Check planet levels
		for(uint i = 1, cnt = MAX_PLANET_LEVEL; i <= cnt; ++i) {
			const PlanetLevel@ lvl = getPlanetLevel(i);
			if(lvl !is null && !lvl.reqs.satisfiedBy(availableResources))
				return i-1;
		}
		return MAX_PLANET_LEVEL;
	}
	...
	uint get_primaryResourceLimitLevel() const {
		if(primaryResource.type is null)
			return 0;
	...
	string getDisabledReason(Object& obj, int id) {
		...
		if(obj.hasSurfaceComponent) {
			...
			else if(obj.population < getPlanetLevelRequiredPop(r.type.level))
				return format(locale::EXPBLOCK_POP, uint(getPlanetLevelRequiredPop(r.type.level)));
		}
	...
	void checkResources(Object& obj, bool wasManual = false) {
		...
		for(uint i = 0; i < cnt; ++i) {
			...
			r.efficiency = resEfficiency * (1.0 + resEfficiencyBonus);
	...
	float get_resourceEfficiency() const {
		return resEfficiency + resEfficiencyBonus;
	}
