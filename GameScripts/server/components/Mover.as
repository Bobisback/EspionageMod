...
class Mover : Component_Mover, Savable {
	Object@ target;
	vec3d destination;
	...
	double moverTick(Object& obj, double time) {
		...
		if(pathNode !is null && (tGoal <= time || tGoal <= 1.0 || (obj.position + obj.velocity).distanceToSQ(dest) < doneRange)) {
			...
			if(tGoal > time) {
				...
				if((leader !is null && leader !is obj) || !rotating) {
	...
	bool moveTo(Object& obj, vec3d point, int& id, bool doPathing = true, bool enterOrbit = true) {
		if(FTL)
			return false;
		if(id > 0 && id == moveID)
			return !moving;
	...
	void stopMoving(Object& obj, bool doPathing = false, bool enterOrbit = true) {
		//TODO: Should stop in a saner way
		int dummy = -1;
		moveTo(obj, obj.position, dummy, doPathing, enterOrbit);
	}
