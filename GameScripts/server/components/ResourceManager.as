...
class ResourceManager : Component_ResourceManager, Savable {
	Mutex popMutex;
	Mutex budgetMutex;
	Mutex ftlMutex;
	Mutex energyMutex;
	...
	int welfareInfluence = 0, welfareEnergy = 0, welfareResearch = 0, welfareHWLabor = 0;
	int storedWelfare = 0;
	...
	double consumeFTL(double amount, bool consumePartial = true) {
	...
	bool get_FTLShortage() const {
		return FTL_Stored <= 0.0001 && FTL_Use > FTL_Income + 0.0001;
	}
	...
	bool isFTLShortage(double amt) const {
		...
		double cons = (FTL_Use + amt) * 60.0;
		double have = FTL_Stored + FTL_Income * 60.0;
		return cons >= have;
	}

	bool consumeFTLUse(double amt) {
		Lock lock(ftlMutex);
		if(FTL_Use + amt <= FTL_Income + 0.0001) {
			FTL_Use += amt;
			return true;
	...
	void resetBudget(Empire& emp) {
		if(Budget_CycleId != 0) {
			...
			int nwf_influence = 0, nwf_energy = 0, nwf_research = 0, nwf_hw_labor = 0;
	...
	void resourceTick(Empire& emp, double time) {
		...
		if(emp.HomeObj is null || !emp.HomeObj.valid || emp.HomeObj.owner !is emp)
			@emp.HomeObj = null;
	
		//Handle FTL income rate
		{
			Lock lock(ftlMutex);
			FTL_Stored = clamp(FTL_Stored + time * (FTL_Income - FTL_Use), 0, max(FTL_Capacity, FTL_Stored));
