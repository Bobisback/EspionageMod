	...
	void regionBuildSupport(Object& obj, uint id, Object& forLeader, const Design@ design) {
		...
		for(uint i = 0, cnt = shipyardList.length; i < cnt; ++i) {
			...
			if(yard.owner !is forLeader.owner)
				continue;
			if(yard.constructingSupport || !yard.canBuildShips)
				continue;
...
#section all
	...
	void setAsteroidResourceLimit(Empire@ forEmp, uint limit) {
		for(uint i = 0, cnt = asteroidList.length; i < cnt; ++i) {
			if(asteroidList[i].owner is forEmp)
				asteroidList[i].setResourceLimit(limit);
		}
	}
...
#section server
	...
	void updateCivilianTrade(Object& region) {
		...
		if(config::ENABLE_CIVILIAN_TRADE == 0.0)
			return;
		uint needRequests = 0;
		int stationLevels = 0;
		for(uint i = 0; i < planetList.length; ++i) {
			...
			int civLimit = 0;
			if(plCount > 100)
				civLimit = 100 + pow(double(plCount - 100), CIVILIAN_LIMIT_POW);
			else
				civLimit = plCount;
	...
	void freeUpCivilian(Object& region, Civilian@ civ) {
		...
		if(bestPlanet !is null && bestTimer > randomd(0.0, CIV_TIMER)) {
			//Reroute the trader
...
#section all
	...
	void regionObjectOwnerChange(Object& thisObj, Object& obj, Empire@ prevOwner, Empire@ newOwner) {
		...
		switch(obj.type) {
			...
			case OT_Asteroid:
				{
				//Update asteroid resource limit
				Asteroid@ roid = cast<Asteroid>(obj);
				if(roid !is null)
					roid.setResourceLimit(1);
				} break;
			case OT_Ship:
				{
