...
class ShipConstructible : Constructible {
	const Design@ design;
	...
	void complete(Object& obj) {
		Ship@ ship = createShip(obj, design);
	...
	uint removeSupports(const Design@ dsg, uint amount, Empire@ refund = null) {
		GroupData@ d;
		@dsg = dsg.mostUpdated();
		for(uint i = 0, cnt = supports.length; i < cnt; ++i) {
			if(supports[i].dsg.mostUpdated() is dsg) {
		...
		if(refund !is null) {
			...
			if(d.orderCycle != -1) {
				...
				refund.refundBudget(getBuildCost(dsg, refd), d.orderCycle);
				d.orderAmount -= refd;
			}
			if(started)
				refund.modMaintenance(-maint, MoT_Construction);
		}
