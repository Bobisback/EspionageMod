...
from construction.ShipConstructible import ShipConstructible;
...
class Construction : Component_Construction, Savable {
	Constructible@[] queue;
	...
	void constructionTick(Object& obj, double time) {
		...
		if(supportDesign !is null) {
			...
			if(subLabor > 0) {
				if(supportLabor < 0) {
					...
					if(supportLabor <= 0) {
						Ship@ ship = createShip(obj, supportDesign, obj.owner, null, false);
		...
		while(queue.length != 0) {
			...
			if(!active.start(obj))
				break;
			if(!active.useLabor(obj, tick))
				break;
			if(active.isUsingLabor(obj))
				usingLabor = true;
	...
	void buildOrbital(Object& obj, int type, vec3d position, Object@ frame = null) {
	...
	void buildStation(Object& obj, const Design@ design, vec3d position, Object@ frame = null) {
		if(capabilities & CC_Orbital == 0)
			return;
		...
		if(!canBuildOrbital(obj, position))
			return;
	...
	void buildDryDock(Object& obj, const Design@ forDesign, float pct) {
		...
		cost = double(cost) * config::DRYDOCK_BUILDCOST_FACTOR;
	...
	void removeSupportShipConstruction(Object& obj, int id, const Design@ dsg, uint amount) {
		...
		cons.removeSupports(dsg, amount, refund = obj.owner);
		deltaConstruction = true;
	}

	void buildAsteroid(Object& obj, Asteroid@ asteroid, uint resId) {
		...
		TradePath path(obj.owner);
		path.generate(getSystem(obj.region), getSystem(asteroid.region));
		if(!path.isUsablePath)
			return;
	...
	void buildConstruction(Object& obj, uint consId, Object@ objTarg = null, vec3d pointTarg = vec3d()) {
		...
		for(uint i = 0, cnt = type.hooks.length; i < cnt; ++i) {
			if(!type.hooks[i].consume(cons.cons, cons.cons.data[i], targs)) {
				for(uint j = 0; j < i; ++j)
					type.hooks[j].reverse(cons.cons, cons.cons.data[j], targs);
				return;
			}
		}
		queueConstructible(obj, cons);
	...
	void startTerraform(Object& obj, Planet@ planet, uint resId) {
		...
		TerraformConstructible cons(obj, planet, res, cost, labor * costFactor);
		queueConstructible(obj, cons);
	}
	...
	void startBuildingConstruction(Object& obj, uint id, vec2i position) {
		...
		if(type is null)
			return;
		queueConstructible(obj, BuildingConstructible(position, type));
	}
	...
	void startRetrofitConstruction(Object& obj, Object@ fleet, int buildCost, double laborCost, int extraMaint) {
		if(capabilities & CC_Ship == 0) {
