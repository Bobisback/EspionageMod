...
final class SurfaceGrid : PlanetSurface {
	int emptyDeveloped = 0;
	int bldMaintenanceRefund = 0;
	int pressureCapTaken = -1;
	double civTimer = 0.0;
	double localMax = 0.0;
	...
	void complete(Object& obj, SurfaceBuilding@ bld) {
		bld.delta = true;
		delta = true;
		...
		for(uint y = 0; y < type.size.y; ++y) {
			for(uint x = 0; x < type.size.x; ++x) {
				...
				if(getFlags(rpos.x, rpos.y) & SuF_Usable == 0) {
					double amt = type.tileBuildCost * biome.buildCost;
			...
			for(uint i = 0, cnt = buildings.length; i < cnt; ++i) {
				...
				for(uint j = 0; j < cnt; ++j) {
					...
					for(uint n = 0, bcnt = cur.type.upgrades.length; n < bcnt; ++n) {
						...
						for(uint j = 0; j < TR_COUNT; ++j) {
							float pres = pressures[j] - saturates[j];
							float sat = type.saturates[j] - cur.type.saturates[j] - sec.type.saturates[j];
							if(sat > pres) {
								newRatio = -1.f;
								break;
							}
	...
	bool checkCivilianConstruction(Object& obj) {
		...
		if(citiesBuilt < uint(pop) && citiesBuilt <= civsBuilt) {
			...
			else if(pressureCapTaken < maxBuildings) {
		...
		for(uint i = 0, cnt = buildings.length; i < cnt; ++i) {
			...
			if(bld.completion < 1.f) {
				...
				if(bld.type.civilian) {
					...
					bld.completion += time * obj.buildingConstructRate * obj.owner.BuildingConstructRate / bld.type.buildTime * obj.owner.ImperialBldConstructionRate;
				bld.delta = true;
