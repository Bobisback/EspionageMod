...
final class SurfaceComponent : Component_SurfaceComponent, Savable {
	...
	void replaceFirstBiomeWith(Object& obj, uint id) {
		...
		Planet@ pl = cast<Planet>(obj);
		if(pl !is null) {
			const PlanetType@ planetType = getBestPlanetType(biomes[0], biomes[1], biomes[2]);
	...
	void destroyBuilding(Object& obj, vec2i pos, bool undevelop =false) {
		grid.destroyBuilding(obj, pos, undevelop=undevelop);
	}
	...
	void buildWithDefense(Object& obj, double time, double defense) {
		...
		if(defenseDesign is null) {
			...
			double laborV = defense * 60.0;
			double optMin = laborV / 10.0, optMax = laborV / 2.0;
			...
			for(uint i = 0, cnt = designs.length; i < cnt; ++i) {
				...
				if(cur <= 0) {
					@defenseDesign = designs[i].newest();
					break;
				}
			}
	...
	void surfaceTick(Object& obj, double time) {
		...
		int newIncome = ceil(grid.resources[TR_Money] * TILE_MONEY_RATE) + popIncome;
