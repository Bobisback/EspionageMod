...
Asteroid@ createAsteroid(const vec3d& position, Region@ region = null) {
	ObjectDesc desc;
...
class AsteroidScript {
	...
	double tick(Asteroid& obj, double time) {
		...
		//Destroy if all our resources run out
		if(obj.nativeResourceCount == 0 && obj.owner.valid)
			obj.destroy();
		...
		//Track the origin object
		Object@ origin = obj.origin;
		if(origin !is null) {
			if(!origin.valid) {
				@origin = null;
			}
			else {
				Empire@ originOwner = origin.owner;
				if(originOwner !is obj.owner && originOwner !is null && originOwner.valid)
					@obj.owner = originOwner;
			}
		}
