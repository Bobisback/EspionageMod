...
class CivilianScript {
	...
	void makeMesh(Civilian& obj) {
		MeshDesc mesh;
		@mesh.model = getCivilianModel(type, obj.radius);
		@mesh.material = getCivilianMaterial(type, obj.radius);
		@mesh.iconSheet = getCivilianIcon(type, obj.radius).sheet;
		mesh.iconIndex = getCivilianIcon(type, obj.radius).index;
		...
		if(pathTarget !is null) {
			...
			if(!leavingRegion) {
				if(intermediate !is null) {
					...
					vec3d leaveDest = prevRegion.position + (nextRegion.position - prevRegion.position).normalized(prevRegion.radius * 0.85) + random3d(0, DEST_RANGE);
					obj.maxAcceleration = ACC_SYSTEM;
