...
void moveToFastest(Ship@ ship, const vec3d& pos) {
	int cost = hyperdriveCost(ship, pos);
	if(ship.owner.FTLStored >= cost) {
...
class Explore : Action {
	...
	bool perform(BasicAI@ ai) {
		...
		if(system !is null) {
			...
			else if(fleet !is null) {
				if(fleet.region !is system && !fleet.hasOrders)
					moveToFastest(fleet, system.position + (fleet.position - system.position).normalized(system.radius * 0.85));
			}
...
final class Armada {
	...
	bool isInCombat(Empire@ empire) const {
		...
		if(target is null || (target.ContestedMask & empire.mask == 0 && target.TradeMask & empire.hostileMask == 0))
			return false;
...
class BuildFleet : Action {
	...
	bool perform(BasicAI@ ai) {
		if(leader !is null) {
			...
			if(!builder.valid || builder.owner !is ai.empire || builder.constructionCount == 0 || builder.laborIncome == 0.0 || !builder.canBuildShips) {
				@builder = null;
				@system = null;
			}
		}
			...
			for(uint i = 0, cnt = ai.factories.length; i < cnt; ++i) {
				...
				if(buildAt.owner is ai.empire && buildAt.hasConstruction && buildAt.canBuildShips &&
					buildAt.constructionCount == 0 && buildAt.laborIncome > TILE_LABOR_RATE * 2.95)
				{
			...
			if(fleets.length < fleetCap && (ai.fleets[FT_Combat].length > 0 || fleets[0].fleetCount >= 3)) {
				...
				if(allBusy) {
					...
					if(fleet !is null) {
						fleets.insertLast(Armada());
						orders.insertLast(null);
						fleets[fleets.length-1].addFleet(fleet);
						newFleet = true;
...
class MakeDesign : Action {
	...
	bool perform(BasicAI@ ai) {
		if(!prepared) {
			...
			double scaleFactor = max(1.0, log(budget / 500.0) / log(2.0));
			...
			switch(type) {
				case DT_Support:
					...
					switch(task) {
						case ST_Filler:
							goalScale = scaleFactor; break;
						case ST_AntiSupport:
							goalScale = scaleFactor * 4.0; break;
						case ST_AntiFlagship:
							weaponCount = 1;
							goalScale = scaleFactor * 9.0; break;
						case ST_Tank:
							weaponCount = 0;
							armorLevel = 1;
							goalScale = scaleFactor * 12.0; break;
						case ST_Supplies:
							goalScale = scaleFactor * 4.0; break;
						default:
							return true;
				...
				case DT_Flagship:
					...
					switch(task) {
						case FST_Scout:
							...
							break;
						case FST_Combat:
							goalScale = scaleFactor * 64.0; break;
						case FST_SuperHeavy:
							weaponCount = 5;
							goalScale = scaleFactor * 128.0; break;
						default:
							return true;
					}
					break;
		...
		for(uint i = 0, cnt = tries.length; i < cnt; ++i) {
			...
			if(type == DT_Flagship) {
				...
				double supportStr = support is null ? 30.0 : support.total(SV_DPS) * dsg.total(SV_ShieldCapacity);
				double supportSize = support is null ? 1.0 : double(support.size);
				
				if(designer.support)
					value *= dsg.total(SV_SupportCapacity) / supportSize * supportStr;
