...
class ExpendResources : Action {
		...
		if(cards.length != 0) {
			...
			for(uint i = 0; i < cnt; ++i) {
				...
				if(vote.startedBy is ai.empire)
					...
					stance = -2;
				else if(vote.startedBy.mask & ai.allyMask != 0)
					stance = 1;
	...
	bool spendResearch(BasicAI@ ai) {
		...
		for(uint i = 0, cnt = techs.length / 6 + 1; i < cnt; ++i) {
	...
	bool perform(BasicAI@ ai) {
		switch(nextStep++ % 9) {
			...
			case 2:
				...
				if(ai.skillEconomy >= DIFF_Medium && !ai.usesMotherships) {
						...
						if(waterWorlds.length > 0) {
							...
							if(waterPl.Population > 0.9999 && waterPl.isPrimaryDestination(null)) {
								...
								for(uint i = 0; i < foods.length; ++i) {
									...
									if(foodPl.region.getTerritory(ai.empire) is terr && foodPl.resourceLevel == 0 && foodPl.Population > 0.9999 && foodPl.isPrimaryDestination(null)) {
										found = true;
			...
			case 3:
				...
				if(ai.skillEconomy >= DIFF_Medium && gameTime > 9.0 * 60.0) {
					auto@ foods = ai.getResourceList(RT_Food, onlyExportable=true);
					int water = ai.getResourceList(RT_Water, onlyExportable=true)[0];
					auto@ level1s = ai.getResourceList(RT_LevelOne);
			...
			case 5:
				...
				
				{
					...
					if(freeBudget > 240 && ai.fleets[FT_Scout].length < 1) {
					...
					else if(ai.needsAltars && freeBudget > 250 && ai.empire.TotalPopulation > ai.empire.getAttribute(getEmpAttribute("AltarSupportedPopulation"))) {
						Planet@ pl = ai.empire.planetList[randomi(0, ai.empire.planetCount - 1)];
						if(pl !is null)
							@queued = ai.requestBuilding(pl, getBuildingType("Altar"));
					}
						...
						else if(freeBudget > buildCost * 2) {
							@queued = ai.requestFleetBuild(FT_Combat);
							return false;
						} 
			...
			case 3:
			{
				...
				if((ai.enemies.length == 0 || ai.behaviorFlags & AIB_QuickToWar != 0) && gameTime > startupTime + 60.0 && ai.empire.SubjugatedBy is null) {
					...
					for(uint i = 0, cnt = getEmpireCount(); i < cnt; ++i) {
						...
						if(other is ai.empire || !other.major)
							continue;
						...
						if(borderMask & other.mask != 0) {
							if(strength - otherStr > 0 && (ai.timeSinceLastExpand > 9.0 * 60.0))
								casusBelli = true;
							else if(strength - otherStr >= 0 && (ai.timeSinceLastExpand > 12.0 * 60.0 || pointStr < -320 || revCount >= 2))
								casusBelli = true;
						}
						
						if(!casusBelli) {
							if(strength - otherStr >= 0 && pointStr < -2250)
								casusBelli = true;
							else if(strength - otherStr >= 0 && ai.timeSinceLastExpand > 20.0 * 60.0)
								casusBelli = true;
				...
				if(ai.enemies.length != 0) {
					...
					if(ai.treatyWaits[other.index] < gameTime) {
						...
						else if((other.points.value + 250) / (ai.empire.points.value + 250) >= 5 && ai.empire.MilitaryStrength < other.MilitaryStrength) {
