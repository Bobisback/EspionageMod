...
class Colonize : Action {
	...
	void sendColonizers(BasicAI@ ai, bool allowAuto = true) {
		if(ai.isMachineRace) {
			//Find planets with sufficient population and ftl them over
			auto@ pl = findSourcePlanet(ai);
			if(pl !is null)
				pl.activateAbilityFor(ai.empire, getAbilityID("MechanoidColonize"), dest);
			return;
		}
		else if(ai.usesMotherships) {
			...
			if(mother !is null && !mother.hasOrders) {
				moveToFastest(mother, dest.position + (mother.position - dest.position).normalize(dest.radius + 2.5));
				mother.addAbilityOrder(mother.findAbilityOfType(getAbilityID("MothershipColonize")), dest, dest.radius + 2.5, append=true);
				@source = mother;
				ai.addIdle(this);
			}
		...
		if(ai.skillEconomy < DIFF_Medium || allowAuto) {
...
class Expand : Action {
	...
	bool perform(BasicAI@ ai) {
		...
		for(uint i = 0, cnt = reg.planets.length; i < cnt; ++i) {
			Planet@ pl = reg.planets[i];
			if(!pl.valid || pl.visibleOwnerToEmp(ai.empire).valid || pl.quarantined)
				continue;
...
class ColonizeByResource : Action {
	...
	string get_state() const {
		if(target is null)
			return "Looking for source of " + resources.length + " resources";
		else
	...
	bool searchForPlanets(BasicAI@ ai) {
		...
		if(regions !is null) {			
			...
			for(uint i = 0, cnt = expansion.planets.length; i < cnt; ++i) {
				if(!expansion.planets[i].visibleOwnerToEmp(ai.empire).valid) {
					@waitingOn = ai.requestColony(expansion.planets[i]);
					@expansion = null;
					break;
				}
			}
