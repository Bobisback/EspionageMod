string formatEmpireName(Empire@ emp) {
	if(emp is null)
		return "(n/a)";
...
string formatMoney(int money, bool colored = false) {
	string text;
	if(money < 0) {
		if(colored)
			text += "[color=#f00]";
	...
	if(am == 0)
		...
		text += standardize(double(am) / 1000.0, true)+"M";
	else
		text += standardize(double(am) / 1000000.0, true)+"B";
