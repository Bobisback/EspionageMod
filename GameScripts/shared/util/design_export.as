...
const Design@ read_design(const string& filename) {
	DesignDescriptor desc;
	read_design(filename, desc);
...
const Hull@ getBestHull(DesignDescriptor& desc, const string& hullTag, Empire@ emp = playerEmpire) {
	...
	for(uint i = 0, cnt = emp.shipset.hullCount; i < cnt; ++i) {
		const Hull@ hull = emp.shipset.hulls[i];
