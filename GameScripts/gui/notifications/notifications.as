...
const double ICON_PULSE_TIME = 1.0;
...
class VoteDisplay : ClassDisplay {
	...
	void draw(NotifyClass@ cls, const recti& iconPos) override {
		...
		if(vote !is null && vote.id != uint(-1)) {
			...
			vote.type.icon.draw(iconPos.padded(30, 0, -10, 0),
					Color(0xffffff70));
			...
			if(vote.totalFor > vote.totalAgainst) {
				...
				timeText = formatShortTime((config::INFLUENCE_PASS_THRES-vote.currentTime) / vote.positiveSpeed);
			}
			else {
				timerColor = Color(0xff0000ff);
				timeText = formatShortTime((vote.currentTime-config::INFLUENCE_FAIL_THRES) / vote.negativeSpeed);
			}
