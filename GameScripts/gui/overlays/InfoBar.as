	...
	void draw() override {
		...
		if(abl.cooldown <= 0) {
			...
			if(abl.cooldown > 0) {
				...
				abl.type.icon.draw(pos, color=colors::White, shader=shader::RadialDimmed);
...
class AutoModeAction : ModeAction {
	...
	uint get() override {
		return obj.getAutoMode();
	}

	void set(Object& obj, uint type) override {
		obj.setAutoMode(type);
	}
};
...
class ActionBar : BaseGuiElement {
	...
	void addScouting(Object@ obj) {
		if(obj.hasLeaderAI && obj.getFleetStrength() < 1000.0) {
			add(ScoutAction(ftl=false));
			if(obj.owner.hasFlingBeacons || canHyperdrive(obj))
