...
const double SUPPLY_CAPTURE_DRAIN = 3000;
...
class RetrofitOption : Option {
	void call() {
		selected.retrofitFleetAt(clicked);
		playOptionSound(sound::order_refresh);
	}
};
...
class TerraformOption : Option, IInputDialogCallback {
	...
	void call() {
		...
		costFactor = selected.terraformCostMod;
...
class FinanceDryDock : Option, IInputDialogCallback {
	...
	void call() {
		...
		price = double(price) * config::DRYDOCK_BUILDCOST_FACTOR;
	...
	void changeCallback(InputDialog@ dialog) {
		...
		int price = ceil(double(getBuildCost(design) * config::DRYDOCK_BUILDCOST_FACTOR) * (nextPct - curPct));
		if(buildAt !is null)
			price += 100;
...
class TargetAbility : SingleSelectionOption {
	int id;
	Object@ target;
	double range;
	Color bg = colors::Invisible;

	TargetAbility(int ID, Object@ targ, double range) {
		id = ID;
		@target = targ;
		this.range = range;
	}
	...
	void call(Object@ selected) {
		if(range == INFINITY || !selected.hasLeaderAI)
			selected.activateAbility(id, target);
		else
			selected.addAbilityOrder(id, target, range, shiftKey);
		playOptionSound(sound::order_attack);
	}

	void draw(GuiListbox@ ele, uint flags, const recti& absPos) override {
		drawRectangle(absPos, bg);
		SingleSelectionOption::draw(ele, flags, absPos);
	}
};

class BuildMiningBase : SingleSelectionOption {
	uint resource;

	BuildMiningBase(uint resId) {
		resource = resId;
	}

	void call(Object@ selected) {
		selected.buildAsteroid(cast<Asteroid>(clicked), resource);
		playOptionSound(sound::order_goto);
	}
};
...
bool openContextMenu(Object& clicked, Object@ selected = null) {
	...
	if(clicked.isPlanet && (clickedOwner is playerEmpire || clicked.isBeingColonized)) {
		if(clicked.hasAutoImports) {
			...
			if(assignLevel != 0 && assignLevel > clickedLevel) {
				Sprite icon;
				if(assignLevel <= 3)
	...
	//Colonization
	if(clicked.isPlanet && playerEmpire.ForbidColonization == 0) {
	...
	if(selected !is null && selected.owner.controlled) {
		...
		//Combat options
		if(hasWeapons(selected) && clickedOwner !is selected.owner && clickedOwner !is null && clickedOwner.valid) {
			Ship@ ship = cast<Ship>(selected);
			if(ship !is null) {
				if(ship.blueprint.canTarget(ship, clicked))
	...
	if(selected !is null && selected.owner is playerEmpire) {
		...
		if(clicked !is null && selected.isShip && selected.hasLeaderAI) {
			if(clicked.isStar && clicked.region.TradeMask & playerEmpire.mask != 0) {
			...
			else if(clicked.isPlanet && clickedOwner is playerEmpire && clicked.supportCount > 0) {
		...
		if(clicked !is null && selected.hasConstruction
			&& selected.canBuildAsteroids && clicked.isAsteroid &&
		...
		if(selected.hasLeaderAI && clicked !is null && clicked.owner is playerEmpire
				&& clicked.hasConstruction && clicked.canBuildShips && selected.region !is null
				&& selected.region is clicked.region) {
			int cost = selected.getRetrofitCost();
			if(cost >= 0) {
				double labor = selected.getRetrofitLabor();
				addOption(menu, selected, clicked, format(locale::RETROFIT_OPTION, formatMoney(cost), standardize(labor, true)), RetrofitOption());
			}
		}
	}
	...
	if((selected is clicked && selected.owner is playerEmpire && selected.isPlanet)
			...
			|| (clickedOwner is playerEmpire && clicked.isOrbital && clicked.hasLeaderAI && clicked.region !is null && clicked.region.TradeMask & playerEmpire.TradeMask.value != 0)
		...
		for(uint i = 0, cnt = abilities.length; i < cnt; ++i) {
			...
			if(abl.type.targets.length == 0) {
				...
				if(!abl.canActivate(targs, ignoreCost=true))
					continue;
				TargetAbility opt(abl.id, clicked, abl.getRange(targs));
