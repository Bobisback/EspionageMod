...
from generic_effects import AddTurret, GenericRepeatHook, ShowsRange;
		...
		for(uint i = 0, cnt = nodes.length; i < cnt; ++i) {
			...
			switch(obj.type) {
				...
				case OT_Planet:
					...
					if(dist > MAX_PLANET_PHYS_SEL_DIST && cast<PlanetIconNode>(cur) is null)
						continue;
					break;
				case OT_Orbital:
		...
		if(obj.isStar) {
			...
			if(obj.hasResources) {
				...
				for(uint i = 0; i < nativeCnt; ++i) {
					...
					else if(!obj.nativeResourceUsable[i])
						color = GlowBeamColor;
				...
				if(obj.owner.controlled) {
					...
					for(uint i = 0; i < availCnt; ++i) {
						Object@ origin = obj.availableResourceOrigin[i];
				...
				{
					uint impCount = obj.queuedImportCount;
					for(uint i = 0; i < impCount; ++i) {
						Object@ origin = obj.queuedImportOrigin[i];
						if(origin is null || origin is obj)
							continue;
