...
class PlanetElement {
	...
	void cache(Resource@ res, bool isPrimary) {
		...
		if(isPrimary) {
			...
			if(mode == PM_TILES && hasSurface) {
				tilesTotal = obj.totalSurfaceTiles;
				tilesUsed = obj.usedSurfaceTiles;
			}
			else {
				tilesTotal = -1;
				tilesUsed = -1;
	...
	void drawTree(const vec2i& startPos) {
		...
		if(isPrimary) {
			...
			if(mode == PM_TILES && tilesTotal != -1) {
				recti area = recti_area(vec2i(startPos.x - 20, iconPos.topLeft.y - 8), vec2i(width+40, 20));
				string txt = format("$1/$2", toString(tilesUsed), toString(tilesTotal));
				Color baseColor(0xaaaaaaff);
				const Font@ ft = font::DroidSans_11_Bold;
				if(tilesTotal < 50)
					@ft = font::DroidSans_8;
				else if(tilesTotal < 90)
					@ft = font::DroidSans_10;
				else if(tilesTotal < 150)
					@ft = font::DroidSans_11;
				else
					baseColor = colors::Green;
				Color color = baseColor.interpolate(colors::Orange, float(tilesUsed) / float(tilesTotal));
...
class PlanetTree : BaseGuiElement {
	...
	void scrollPane(int dx, int dy) {
		scroll -= vec2d(double(dx), double(dy));
		scroll.x = clamp(scroll.x, 0.0, max(double(totalWidth - size.width), 0.0));
		scroll.y = clamp(scroll.y, 0.0, max(double(totalHeight - size.height + 60.0), 0.0));
	}
...
class PlanetsTab : Tab {
	...
	void tick(double time) override {
		...
		if(!tree.isDragging) {
			if(mousePos.x <= AbsolutePosition.topLeft.x + 2) {
				tree.scrollPane(+5, 0);
			}
			else if(mousePos.x >= AbsolutePosition.botRight.x - 2) {
				tree.scrollPane(-5, 0);
			}
			if(mousePos.y <= AbsolutePosition.topLeft.y + 2) {
				tree.scrollPane(0, +5);
			}
			else if(mousePos.y >= AbsolutePosition.botRight.y - 2) {
				tree.scrollPane(0, -5);
			}
		}
	}
