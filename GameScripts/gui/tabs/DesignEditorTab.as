...
from tabs.tabbar import browseTab, popTab;
...
class CreateSubsystemAction : Action {
	...
	void act(DesignData@ data) {
		...
		else if(def.hasTag(ST_HexLimitArc)) {
...
class RandomConfig : InputDialogCallback {
	...
	void prompt(IGuiElement@ parent, const Design@ dsg) {
		...
		if(dsg.hull.hasTag("Flagship")) {
			if(playerEmpire.isUnlocked(subsystem::Hyperdrive))
...
class Construction : BaseGuiElement {
	...
	Construction(DesignEditor@ ed, Alignment@ align) {
		...
		@randomButton = GuiButton(this, Alignment(Right-650-36, Top+2, Width=32, Height=32), Sprite(spritesheet::MenuIcons, 4, Color(0xff8040ff)));
		...
		@paintButton = GuiButton(this, Alignment(Right-446, Top+2, Width=32, Height=32), icons::Paint);
		...
		@moveButton = GuiButton(this, Alignment(Right-410, Top+2, Width=32, Height=32), icons::Move);
		...
		@dropButton = GuiButton(this, Alignment(Right-374, Top+2, Width=32, Height=32), icons::Eyedrop);
		...
		@display = GuiBlueprint(this, Alignment(Left+258+12, Top+32+12, Right-258-12, Bottom-44));
	...
	void updateStatsPos() {
		...
		if(statsPanel.size.height - globalStats.size.height - 30 > statsPopup.size.height) {
			...
			pos.x = display.rect.botRight.x - statsPopup.size.width;
			pos.y = display.rect.topLeft.y;
			if(mousePos.x > display.absolutePosition.center.x)
				pos.x = display.rect.topLeft.x;
		}
		...
		if(paintTool.def !is def) {
			...
			for(uint i = 0, cnt = selectors.length; i < cnt; ++i) {
				if(selectors[i].contains(def)) {
					syslist.openSection(i, snap=false);
					break;
				}
			}
		}
