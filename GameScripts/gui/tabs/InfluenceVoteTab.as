...
class InfluenceVoteTab : Tab {
	...
	void update(int voteId) {
		...
		for(uint i = 0, cnt = boxes.length; i < cnt; ++i) {
			boxes[i].set(vote.empireVotes[i], vote.isPresent(boxes[i].emp),
		...
		if(vote.totalFor > vote.totalAgainst) {
			...
			timeText = formatTime((config::INFLUENCE_PASS_THRES-vote.currentTime) / vote.positiveSpeed);
		}
		else {
			...
			timeText = formatTime((vote.currentTime-config::INFLUENCE_FAIL_THRES) / vote.negativeSpeed);
		}
		...
		//Update log
		if(lastLog != int(vote.events.length)) {
			//Log events
	...
	void addEffect(InfluenceVoteEvent@ evt) {
		EffectButton@ btn;
		...
		btn.setIcon(evt.cardEvent.card.type.icon);
		btn.color = evt.emp.color;
		btn.card = evt.cardEvent.card;
		setMarkupTooltip(btn, "[font=Medium]"+formatEmpireName(evt.emp)+": [/font]"+evt.cardEvent.card.formatTooltip(showUses=false), width=450);
			...
			if(event.caller is backButton) {
				...
				for(uint i = 0, cnt = effects.length; i < cnt; ++i) {
					if(effects[i] is event.caller)
						GuiInfluenceCardPopup(this, event.caller, events[i].cardEvent.card, vote = vote, playable = false);
				}
			}
		}
...
class DelegationBox : BaseGuiElement {
	...
	DelegationBox(IGuiElement@ parent, Empire@ empire) {
		...
		@leaderIcon = GuiSprite(picture, Alignment(Left, Bottom-26, Left+26, Bottom));
