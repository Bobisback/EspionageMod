...
void init() {
	...
	version.text = "Version: "+SCRIPT_VERSION;
...
class MenuAction : GuiListElement {
	...
	void draw(GuiListbox@ ele, uint flags, const recti& absPos) override {
		...
		if(icon.valid) {
			...
			icon.draw(recti_area(absPos.topLeft + vec2i(textOffset.x, 4), vec2i(iSize, iSize)));
