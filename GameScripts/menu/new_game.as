...
from elements.Gui3DObject import drawLitModel;
...
class NewGame : BaseGuiElement {
	...
	NewGame() {
		...
		@mapsButton = GuiButton(gameHeader, Alignment(Left, Top, Width=200, Height=40));
		...
		mapsButton.style = SS_PageTab;
		mapsButton.color = Color(0xff003fff);
		...
		for(uint i = 0, cnt = GAME_SETTINGS_PAGES.length; i < cnt; ++i) {
			...
			auto@ button = GuiButton(gameHeader, Alignment(Left+200+(i*200), Top, Width=200, Height=40));
			button.color = page.color;
			button.text = page.header;
			button.buttonIcon = page.icon;
			button.toggleButton = true;
			button.pressed = false;
			button.font = FT_Medium;
			button.style = SS_PageTab;
...
void drawRace(const Skin@ skin, const recti& absPos, const string& name,
	...
	if(traits !is null) {
		...
		for(uint i = 0, cnt = traits.length; i < cnt; ++i) {
			auto@ trait = traits[i];
			if(trait.unique == "FTL") {
				trait.icon.draw(recti_area(ftlPos.topLeft, vec2i(absPos.height, absPos.height)));
				ftlPos.topLeft.x += absPos.height+8;
				normal.draw(text=trait.name, pos=ftlPos);
			}
			else if(showTraits) {
				traits[i].icon.draw(pos);
				pos -= vec2i(24, 0);
			}
		}
	}
...
class RaceChooser : GuiOverlay {
	...
	void selectRace(uint select) {
		...
		auto@ preset = getRacePreset(select);
...
class EmpireSetup : BaseGuiElement, IGuiCallback {
	...
	void showDifficulties() {
		...
		menu.addOption(ChangeDifficulty(this, 5, locale::DIFF_SAVAGE));
	...
	void forceAITraits(EmpireSettings& settings) {
		settings.chooseTrait(getTrait("Hyperdrive"));
		if(settings.hasTrait(getTrait("Mechanoid")) || settings.hasTrait(getTrait("Verdant")) || settings.hasTrait(getTrait("Linked"))) {
			switch(randomi(0,3)) {
				case 0: settings.chooseTrait(getTrait("Terrestial")); break;
				case 1: settings.chooseTrait(getTrait("Evangelical")); break;
				case 2: settings.chooseTrait(getTrait("Devout")); break;
				case 3: settings.chooseTrait(getTrait("StarChildren")); break;
			}
		}
	}
	...
	void resetName() {
		name.text = format(localize(empireNames.generate()), settings.raceName);
	...
	void drawElement(uint index, const recti& pos) override {
		...
		if(index < items.length) {
			...
			if(hull !is null) {
				...
				drawLitModel(hull.model, hull.material, pos+vec2i(-4,0), rot, 1.9);
				clearClip();
			}
...
class TraitDisplay : BaseGuiElement {
	...
	TraitDisplay(IGuiElement@ parent) {
		...
		@icon = GuiSprite(this, Alignment(Left+14, Top+12, Left+106, Bottom-12));

		@name = GuiMarkupText(this, Alignment(Left+122, Top+8, Right-168, Top+38));
...
class TraitsWindow : BaseGuiElement {
	...
	TraitsWindow(EmpireSetup@ setup) {
		...
		categories.itemHeight = 34;
		categories.style = SS_PlainOverlay;
		categories.addItem(locale::RACE_PROFILE);
		...
		for(uint i = 0, cnt = getTraitCategoryCount(); i < cnt; ++i) {
			...
			for(uint n = 0, ncnt = getTraitCount(); n < ncnt; ++n) {
				if(getTrait(n).category is cat && getTrait(n).available) {
					hasTraits = true;
					break;
				}
			}
			if(hasTraits) {
				categories.addItem(cat.name);
	...
	void update() {
		...
		if(traitPanel.visible) {
			...
			for(uint i = 0, cnt = getTraitCount(); i < cnt; ++i) {
				...
				int needH = max(140, disp.description.renderer.height+42);
				if(needH != 140) {
