...
#section server
	...
	void tick(Object& obj, any@ data, double tick) const override {
		if(obj.isPlanet) {
			...
			double newAmt = obj.population * arguments[0].decimal;
			if(amt != newAmt) {
...
#section server
	...
	void tick(Object& obj, any@ data, double time) const override {
		...
		//Check rate
		double curRate = obj.getResourceProduction(resource.integer);
		if(curRate < 0) {
			double mod = min(-curRate, info.taken);

			obj.modResource(resource.integer, +mod);
			obj.modResource(income.integer, -mod * factor.decimal);

			info.taken -= mod;
			info.given -= mod * factor.decimal;
		}
		else if(curRate > 0) {
			double mod = curRate;
			if(maximum_converted.decimal >= 0)
				mod = min(mod, maximum_converted.decimal - info.taken);
...
final class AddToNativeData {
	Object@ current;
	any data;
};

class AddToNativeResource : GenericEffect {
	GenericEffect@ hook;
