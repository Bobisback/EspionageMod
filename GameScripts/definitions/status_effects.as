...
#section server
	...
	bool onTick(Object& obj, Status@ status, any@ data, double time) override {
		...
		if(currentTotal < target - 0.001) {
			uint index = randomi(1, TR_COUNT-1);
			for(uint i = 1; i < TR_COUNT && currentTotal < target; ++i) {
...
class TriggerCreate : StatusHook {
	BonusEffect@ eff;
...
class TriggerColonized : StatusHook {
	BonusEffect@ eff;
	Argument hook(AT_Hook, "bonus_effects::BonusEffect", doc="Hook to call.");
...
class OnOriginEmpire : StatusHook {
	EmpireEffect@ eff;
...
class RemoveOnWarWithOriginEmpire : StatusHook {
