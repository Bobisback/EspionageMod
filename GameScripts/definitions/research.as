...
export TechnologyType, TechnologyNode;
...
final class TechnologyNode : Serializable, Savable {
	int id = -1;
	vec2i position;
	...
	double getPointCost(Empire@ emp = null) {
		return type.pointCost;
	}

	double getTimeCost(Empire@ emp = null) {
...
final class TechnologyGrid : Savable {
	HexGridi indices;
	vec2i minPos;
	vec2i maxPos;
...
final class TechnologyGridSpec {
	uint id;
	string ident;
...
void parseLine(string& line, TechnologyType@ type) {
	//Hook line
	auto@ hook = cast<ITechnologyHook>(parseHook(line, "research_effects::", instantiate=false));
	if(hook !is null)
...
void loadGrid(TechnologyGridSpec@ grid, ReadFile@ file, const string& key, const string& value) {
	...
	grid.def_positions.insertLast(vec2i(toInt(pos[0]), toInt(pos[1])));
}
