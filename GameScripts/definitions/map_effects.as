...
#section server
Planet@ spawnPlanetSpec(const vec3d& point, const string& resourceSpec, bool distributeResource = true) {
	MakePlanet plHook;
	plHook.initClass();
...
#section server
	void trigger(SystemData@ data, SystemDesc@ system, Object@& current) const override {
		...
		if(data is null && distribute)
			@resource = getDistributedResource();
		if(cur.isAsteroid) {
			if(resource is null && distribute)
				@resource = getDistributedAsteroidResource();
			if(resource !is null)
				markResourceUsed(resource);
...
class MakeAsteroid : MapHook {
	...
	Argument type("Resource", AT_Custom, "distributed", doc="Type or types of resources to place on the asteroid.");
	Argument extraChance("Distribution Chance", AT_Decimal, "0.4", doc="For distributed resources, chance to add additional resource. Repeats until failure.");
	array<const ResourceType@> resPossib;
	bool distribute = false;
...
#section server
	void trigger(SystemData@ data, SystemDesc@ system, Object@& current) const override {
		...
		if(distribute) {
			...
			while(randomd() < extraChance.decimal);
		}
		else {
			const ResourceType@ resource;
			if(resPossib.length > 0) {
...
//MakeAsteroidBelt(<Count> = 8, <Resource Chance> = 0.4)
// Create a new asteroid belt made of <Count> asteroids.
// These asteroids get distributed resources with <Resource Chance> per additional resource
class MakeAsteroidBelt : MapHook {
	...
	Argument extraChance("Resource Chance", AT_Decimal, "0.4", doc="Chance to add additional resource. Repeats until failure on each asteroid.");
...
#section server
	...
	roidHook.distribute = false;
	roidHook.resPossib.length = 0;
