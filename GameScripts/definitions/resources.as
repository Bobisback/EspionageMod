...
int getBuildCost(const Design@ dsg, int count = 1) {
	double build = 0;
	uint cnt = dsg.subsystemCount;
	for(uint i = 0; i < cnt; ++i) {
		const Subsystem@ sys = dsg.subsystems[i];
...
void parseLine(string& line, ResourceType@ r) {
	if(line.findFirst("(") == -1) {
		//Pressure line
		...
		//Hook line
		auto@ hook = cast<IResourceHook>(parseHook(line, "resource_effects::", instantiate=false));
		if(hook !is null)
