...
#section server
from influence_global import modInfluenceStackSize, getLastInfluenceVoteId, getActiveInfluenceVotes, getInfluenceVotesSince, getLastInfluenceEffectId, getInfluenceEffectsSince;
#section all

#section server
final class VisionEffect {
...
#section server
	...
	bool onTick(InfluenceEffect@ eff, double time) const override {
		...
		for(uint i = 0, cnt = list.length; i < cnt; ++i) {
			auto@ vote = list[i];
			if(vote.startedBy !is eff.owner && vote.startedBy.valid)
...
#section server
	...
	bool onTick(InfluenceEffect@ eff, double time) const override {
		...
		for(uint i = 0, cnt = list.length; i < cnt; ++i) {
			auto@ effect = list[i];
			if(effect.owner !is eff.owner && effect.owner.valid)
