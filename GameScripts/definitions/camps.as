...
void parseLine(string& line, CampType@ c) {
	...
	if(line.findFirst("(") == -1) {
		...
		//Hook line
		auto@ hook = cast<IPickupHook>(parseHook(line, "pickup_effects::", instantiate=false));
		if(hook !is null)
...
void addCreepCamp(CampType@ type) {
	...
	for(uint i = 0, cnt = type.pickups.length; i < cnt; ++i) {
		addPickupType(type.pickups[i]);
		type.pickupFrequency += type.pickups[i].frequency;
	}
}
