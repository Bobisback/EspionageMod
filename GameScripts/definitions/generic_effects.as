...
// All asteroids in the system can be mined fully.
class UnlimitedAsteroidSystem : GenericEffect {
	Document doc("Asteroids in the system this effect is in can be mined for more than one resource at a time.");
...
#section server
	void tick(Object& obj, any@ data, double time) const override {
		...
		Empire@ owner = obj.owner;
		if(region !is null && owner !is null && owner.valid)
			region.setAsteroidResourceLimit(owner, 1000000);
	}

	void ownerChange(Object& obj, any@ data, Empire@ prevOwner, Empire@ newOwner) const override {
		Region@ region = obj.region;
		if(region !is null && prevOwner !is null && prevOwner.valid)
			region.setAsteroidResourceLimit(prevOwner, 1);
	}
	...
	void regionChange(Object& obj, any@ data, Region@ fromRegion, Region@ toRegion) const {
		...
		if(owner !is null && owner.valid) {
			if(fromRegion !is null)
				fromRegion.setAsteroidResourceLimit(owner, 1);
		}
	}
	...
	void disable(Object& obj, any@ data) const override {
		...
		Empire@ owner = obj.owner;
		if(region !is null && owner !is null && owner.valid)
			region.setAsteroidResourceLimit(owner, 1);
	}
#section all
};
...
class EnableModifier : GenericEffect {
...
#section server
	...
	void disable(Object& obj, any@ data) const override {
		...
		if(ids !is null && obj.owner !is null && obj.owner.valid)
			mod.remove(obj.owner, ids);
	}
...
class TriggerPeriodic : GenericEffect {
	BonusEffect@ hook;
...
class TriggerOnAttributeIncrease : GenericEffect {
	BonusEffect@ hook;
...
#section server
	void enable(Object& obj, any@ data) const override {
		...
		if(obj.owner !is null)
			amount = obj.owner.getAttribute(attribute.integer);
		data.store(amount);
	}
...
class OnEmpireAttributeGTE : GenericEffect {
	BonusEffect@ hook;
...
class OnEmpireAttributeLT : GenericEffect {
	BonusEffect@ hook;
...
class IfNearFriendlyPlanets : IfHook {
...
#section server
	bool condition(Object& obj) const override {
		if(!obj.isPlanet)
			return false;
		int lv = obj.primaryResourceLimitLevel;
		if(exact.boolean)
			return lv == level.integer;
		return lv >= level.integer;
	}
...
class IfPopulationBelow : IfHook {
...
class IfCoversPopulation : IfHook {
...
class AllowConstruction : GenericEffect {
	...
	Argument roids("Asteroids", AT_Boolean, "False", doc="Allow asteroid mine construction.");
...
#section server
	void enable(Object& obj, any@ data) const override {
		...
		Region@ reg = obj.region;
		if(reg !is null && arguments[0].boolean)
...
#section server
	...
	void tick(Object& obj, any@ data, double time) const override {
		...
		while(progress >= 1.0) {
			...
			colShip.maxAcceleration = 2.5 * obj.owner.ModSpeed.value;
