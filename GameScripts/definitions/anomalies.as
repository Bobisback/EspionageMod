...
class AnomalyHook : Hook, IAnomalyHook {
	void init(AnomalyType@ type) {}
	void choose(Anomaly@ obj, Empire@ emp, Targets@ targets) const {}
...
void parseLine(string& line, AnomalyType@ anomaly, AnomalyResult@ result) {
	...
	if(line.findFirst("(") == -1) {
		...
		//Hook line
		auto@ hook = cast<IAnomalyHook>(parseHook(line, "anomaly_effects::", instantiate=false));
		if(hook !is null) {
			if(result !is null)
