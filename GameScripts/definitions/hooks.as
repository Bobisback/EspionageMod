...
#section server
	bool init(Design& design, Subsystem& subsystem, StringList& arglist, DoubleList& values) const {
		...
		initClass();
		if(!parse("--", args)) {
			error("Error in hook "+formatHook());
			return false;
		}
...
#section all
	...
	bool parse(const string& name, array<string>& args) {
		if(args.length < requiredArguments) {
			error(" Error: mismatched argument count, expected "+arguments.length+" arguments.");
			return false;
		}
		bool inNamed = false;
		uint i = 0;
		for(uint cnt = arguments.length, acnt = args.length; i < cnt && i < acnt; ++i) {
			...
			if(eqPos != -1 && (brkPos == -1 || eqPos < brkPos)) {
				...
				if(index == uint(-1)) {
					error(" Error: Could not find argument with name: "+name);
					return false;
				}
				else {
				...
				//Positional argument
				if(inNamed) {
					error(" Error: positional arguments cannot follow named ones.");
					return false;
				}
				//Variable arguments
		...
		for(uint i = 0, cnt = requiredArguments; i < cnt; ++i) {
			if(!arguments[i].filled) {
				error(" Error: no value for argument "+i+" ("+arguments[i].argName+")");
				return false;
			}
		}
		return true;
	...
	bool initTargets(Targets@ targets) {
		...
		for(uint i = 0, cnt = arguments.length; i < cnt; ++i) {
			...
			if(index >= targets.targets.length) {
				...
				error("Could not find target: "+arg.str);
				return false;
			}
			...
			if(arg.integer2 != int(TT_Any)) {
				if(targ.type != TargetType(arg.integer2)) {
					error("Target "+targ.name+" is not of appropriate type.");
					return false;
				}
			}
			else {
