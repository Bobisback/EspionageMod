...
class BuildingType {
	...
	string getTooltip(bool showName = true, bool showCost = true, bool showSize = true, bool showAffinity = true, Object@ valueObject = null, bool isOption = false) const {
		...
		if(showCost && !civilian) {
			...
			tt += format(locale::BLD_TT_COST, formatMoney(buildCostEst, maintainCostEst));
			...
			if(showSize && (size.x > 1 || size.y > 1))
				tt += "[nl/]\n"+format(locale::BLD_TT_SIZE, toString(size.x), toString(size.y));
		}
		return tt;
	}
...
class SurfaceBuilding : Serializable, Savable {
	vec2u position;
